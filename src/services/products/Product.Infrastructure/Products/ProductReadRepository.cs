...
namespace Product.Infrastructure.Products
{
    public class ProductReadRepository : IProductReadRepository
    {
        ...
        public async Task<List<Domain.Products.Product>> GetAllAsync()
        {
            return await _dbContext.Products.Include(p => p.Category).ToListAsync();
        }

        public async Task<Domain.Products.Product> GetAsync(int id) =>
            await _dbContext.Products.FirstOrDefaultAsync(p => p.Id == id) ?? new Domain.Products.Product { };



        public async Task<Domain.Products.Product> GetAsyncNoTracking(int id)
        {
